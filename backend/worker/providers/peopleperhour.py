"""PeoplePerHour provider - scrapes freelancer/agency listings."""
import logging
import re
...
class PeoplePerHourProvider(BaseProvider):
    name = "PeoplePerHour"
    slug = "peopleperhour"
    description = "PeoplePerHour.com - freelance agency directory"
    requires_browser = False
    supported_countries = ["*"]
    supported_industries = ["design", "development", "marketing", "writing"]
    ...
    async def search(
        ...
        **kwargs
        ...
        urls_to_try = [
            f"{self.BASE_URL}/freelance-agencies",
            f"{self.BASE_URL}/search?q={search_term}",
        ]

        if location:
            urls_to_try.insert(0, f"{self.BASE_URL}/freelance-agencies?location={location.replace(' ', '+')}")
        ...
        async with httpx.AsyncClient(timeout=20, follow_redirects=True, verify=False) as client:
            ...
            for url in urls_to_try:
                try:
                    ...
                    logger.debug(f"PeoplePerHour URL failed {url}: {e}")
                    continue
    ...
    def _parse_listing_page(self, html: str, default_location: str) -> List[NormalizedLead]:
        ...
        cards = soup.select('[class*="card"], [class*="agency"], [class*="provider"], article')
        if not cards:
        ...
        for card in cards[:50]:
            try:
                ...
                results.append(NormalizedLead(
                    ...
                    raw_data={"source": "peopleperhour", "url": link}
                ))

            except Exception as e:
                logger.debug(f"Failed to parse PeoplePerHour card: {e}")
                continue
