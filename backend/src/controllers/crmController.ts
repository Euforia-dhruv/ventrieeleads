...
export async function getLeadTimeline(req: Request, res: Response): Promise<void> {
  try {
    ...
    const [activities, changes, tasks] = await Promise.all([
      pool.query(
        "SELECT *, 'activity' as entry_type FROM activities WHERE lead_id = $1 ORDER BY created_at DESC LIMIT 50",
        [id],
      ),
      pool.query(
      ...
      pool.query(
        "SELECT *, 'task' as entry_type FROM lead_tasks WHERE lead_id = $1 ORDER BY created_at DESC LIMIT 50",
        [id],
      ),
    ]);
...
export async function getLeadTasks(req: Request, res: Response): Promise<void> {
  try {
    const pool = getPool();
    const { id } = req.params;
    const result = await pool.query('SELECT * FROM lead_tasks WHERE lead_id = $1 ORDER BY due_date ASC NULLS LAST', [
      id,
    ]);
    res.json({ success: true, data: result.rows });
  } catch (error) {
...
export async function createLeadTask(req: Request, res: Response): Promise<void> {
  try {
    ...
    await pool.query(`INSERT INTO activities (lead_id, action, description) VALUES ($1, 'task_created', $2)`, [
      id,
      `Task created: ${title}`,
    ]);
...
export async function updateLeadTask(req: Request, res: Response): Promise<void> {
  try {
    ...
    const result = await pool.query(`UPDATE lead_tasks SET ${sets.join(', ')} WHERE id = $${idx} RETURNING *`, values);
...
export async function addLeadNote(req: Request, res: Response): Promise<void> {
  try {
    ...
    await pool.query(`INSERT INTO activities (lead_id, action, description) VALUES ($1, 'note_added', $2)`, [id, note]);
