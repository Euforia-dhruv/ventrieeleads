...
export async function getDiscoveryIntelligence(_req: Request, res: Response): Promise<void> {
  try {
    ...
    const [totalComp, totalLoc, totalInd, coveredLoc, coveredInd, countries] = await Promise.all([
      ...
      pool.query(
        "SELECT COUNT(*) FROM locations WHERE is_deleted = FALSE AND is_active = TRUE AND location_type = 'city'",
      ),
      pool.query(
        'SELECT COUNT(*) FROM industries WHERE is_deleted = FALSE AND is_active = TRUE AND parent_id IS NOT NULL',
      ),
      pool.query(
    ...
    const [newWeek, newMonth, prevWeek] = await Promise.all([
      ...
      pool.query('SELECT COUNT(*) FROM companies WHERE is_deleted = FALSE AND created_at > $1 AND created_at <= $2', [
        twoWeeksAgo,
        weekAgo,
      ]),
    ]);
    ...
    const confidence = Math.min(
      ...
      Math.round(
        coverageScore * 0.3 +
          Math.min(velocity, 50) * 2 * 0.3 +
          (Math.min(tc, 1000) / 10) * 0.2 +
          industryCoverage * 0.2,
      ),
    );
  ...
  try {
    ...
    const scored = Object.values(providers).map((p: any) => {
      ...
      const composite =
        Math.round(
          (health * 0.25 +
            latency * 0.15 +
            Math.min(quality / 20, 1) * 0.2 +
            dup * 0.15 +
            freshness * 0.1 +
            Math.min(p.countries / 5, 1) * 0.15) *
            1000,
        ) / 1000;
      return {
        provider: p.slug,
        scores: {
          health_score: health,
          coverage_score: Math.min(p.countries / 5, 1),
          quality_score: Math.min(quality / 20, 1),
    ...
    if (scored.length === 0) {
      recommendations.push({
        ...
        description: 'Run campaigns to collect metrics.',
        confidence: 1.0,
        priority: 10,
        type: 'setup',
      });
      ...
      scored
        .filter((s: any) => s.scores.health_score < 0.7)
        .forEach((s: any) => {
          recommendations.push({
...
export async function getMarketIntelligence(_req: Request, res: Response): Promise<void> {
  try {
    ...
    const cityData = citiesResult.rows.map((r: any) => ({
      ...
      growth_rate:
        r.total_companies > 0 ? Math.round((parseInt(r.new_this_week) / parseInt(r.total_companies)) * 1000) / 10 : 0,
    ...
    const industryData = indResult.rows.map((r: any) => ({
      ...
      new_this_week: parseInt(r.new_this_week),
      growth_rate:
        parseInt(r.total_companies) > 0
          ? Math.round((parseInt(r.new_this_week) / parseInt(r.total_companies)) * 1000) / 10
          : 0,
    }));
    ...
    res.json({
      success: true,
      data: {
        fastest_growing_cities: cityData
          .filter((c: any) => c.total_companies >= 3)
          .sort((a: any, b: any) => b.growth_rate - a.growth_rate)
          .slice(0, 10),
        fastest_growing_industries: [...industryData]
          .sort((a: any, b: any) => b.growth_rate - a.growth_rate)
          .slice(0, 10),
        most_competitive_industries: industryData.slice(0, 10),
        least_competitive_industries: [...industryData]
          .filter((i: any) => i.total_companies > 0)
          .sort((a: any, b: any) => a.total_companies - b.total_companies)
          .slice(0, 10),
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          avg_score:
            scored.length > 0
              ? Math.round(scored.reduce((s: number, x: any) => s + x.scores.overall, 0) / scored.length)
              : 0,
  ...
  try {
    ...
    const countries = result.rows.map((r: any) => {
      ...
      return {
        ...
        total_companies: tc,
        coverage_jobs: cj,
        lead_density: cities > 0 ? Math.round((tc / cities) * 10) / 10 : 0,
  ...
  try {
    ...
    const predictions = cityResult.rows.map((r: any) => {
      ...
      const confidence =
        Math.min(
          0.95,
          Math.round((existing > 0 ? 0.3 : 0.1) + (r.latitude ? 0.3 : 0) + (r.population ? 0.2 : 0.1) + 0.2) * 100,
        ) / 100;
  ...
  try {
    ...
    if (failureRate > 0.3)
      recommendations.push({
        ...
        description: `${Math.round(failureRate * 100)}% failure rate. Review providers.`,
        confidence: 0.9,
        priority: 9,
      });
    if (q > r * 10)
      recommendations.push({
        title: 'Queue backlog',
        description: `${q} queued vs ${r} running. Increase concurrency.`,
        confidence: 0.85,
        priority: 8,
      });
    if (avgRuntime > 60000)
      recommendations.push({
        ...
        description: `Avg ${Math.round(avgRuntime / 1000)}s. Check provider latency.`,
        confidence: 0.8,
        priority: 7,
      });
    if (successRate > 0.95 && total > 10)
      recommendations.push({
        title: 'High success rate',
        description: `${Math.round(successRate * 100)}% success. Safe to increase throughput.`,
        confidence: 0.75,
        priority: 5,
      });
    ...
    res.json({
      ...
      data: {
        ...
        top_errors: errorsResult.rows.map((e: any) => ({
          error: e.error_message?.slice(0, 100),
          count: parseInt(e.cnt),
        })),
        recommendations,
      },
    });
...
export async function getEconomicsData(_req: Request, res: Response): Promise<void> {
  try {
    ...
    const [companies, leads, opportunities, searchJobs, audits, campaigns] = await Promise.all([
      ...
      pool.query(
        'SELECT COALESCE(SUM(total_businesses),0) as tb, COALESCE(SUM(unique_businesses),0) as ub, COALESCE(SUM(provider_requests),0) as pr, COALESCE(SUM(estimated_cost_usd),0) as cost FROM discovery_campaigns WHERE is_deleted = FALSE',
      ),
    ]);
    ...
    res.json({
      ...
      data: {
        ...
        costs: {
          ...
          cost_per_enriched:
            parseInt(camp.ub) > 0 ? Math.round((parseFloat(camp.cost) / parseInt(camp.ub)) * 10000) / 10000 : 0,
        ...
        efficiency: {
          ...
          enrichment_rate:
            parseInt(camp.tb) > 0 ? Math.round((parseInt(camp.ub) / parseInt(camp.tb)) * 1000) / 1000 : 0,
