...
export async function getPipelineOverview(_req: Request, res: Response): Promise<void> {
  try {
    ...
    for (const stage of stages.rows) {
      ...
      stageData.push({
        stage_id: stage.id, stage_name: stage.name, slug: stage.slug,
        color: stage.color, icon: stage.icon,
        count, value_min: parseInt(entry.vmin), value_max: parseInt(entry.vmax),
...
export async function transitionLeadPipeline(req: Request, res: Response): Promise<void> {
  try {
    ...
    const toStage = await pool.query(
      "SELECT id FROM pipeline_stages WHERE slug = $1 AND is_deleted = FALSE",
      [to_stage]
    );
    ...
    let entry = await pool.query(
      "SELECT * FROM lead_pipeline WHERE lead_id = $1 AND is_deleted = FALSE",
      [leadId]
    );
    ...
    if (entry.rows.length === 0) {
      ...
      await pool.query(
        "UPDATE lead_pipeline SET stage_id = $1, confidence = $2, updated_at = NOW() WHERE id = $3",
        [toStage.rows[0].id, confidence, entry.rows[0].id]
      );
    }
...
export async function getPipelineStats(_req: Request, res: Response): Promise<void> {
  try {
    ...
    res.json({
      success: true,
      data: {
        total: t, active: parseInt(active.rows[0].count),
        won: w, lost: l,
...
export async function createAutomationRule(req: Request, res: Response): Promise<void> {
  try {
    const pool = getPool();
    const { name, description, trigger_event, conditions, actions, cooldown_hours = 24, max_executions = 100, priority = 5 } = req.body;
    ...
    `, [name, description || null, trigger_event, JSON.stringify(conditions), JSON.stringify(actions), cooldown_hours, max_executions, priority]);
...
export async function toggleAutomationRule(req: Request, res: Response): Promise<void> {
  try {
    const pool = getPool();
    await pool.query(
      "UPDATE automation_rules SET is_active = NOT is_active, updated_at = NOW() WHERE id = $1",
      [req.params.id]
    );
...
export async function deleteAutomationRule(req: Request, res: Response): Promise<void> {
  try {
    const pool = getPool();
    await pool.query(
      "UPDATE automation_rules SET is_deleted = TRUE, updated_at = NOW() WHERE id = $1",
      [req.params.id]
    );
    res.json({ success: true });
  } catch (error) {
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        success_rate: Math.round(parseInt(success.rows[0].count) / Math.max(parseInt(execs.rows[0].count), 1) * 1000) / 10,
      },
    });
  } catch (error) {
...
export async function getSystemOverview(_req: Request, res: Response): Promise<void> {
  try {
    ...
    const [companies, leads, activeSearches, recentCompleted, recentFailed, recentQueued] = await Promise.all([
      ...
      pool.query("SELECT COUNT(*) FROM campaign_jobs WHERE created_at > $1 AND status = 'completed' AND is_deleted = FALSE", [dayAgo]),
      pool.query("SELECT COUNT(*) FROM campaign_jobs WHERE created_at > $1 AND status = 'failed' AND is_deleted = FALSE", [dayAgo]),
    ...
    res.json({
      ...
      data: {
        ...
        last_24h: {
          completed: rc, failed: rf,
          queued: parseInt(recentQueued.rows[0].count),
          success_rate: Math.round(rc / Math.max(rc + rf, 1) * 1000) / 10,
        },
        pipeline: {
          total: parseInt(p.total), active: parseInt(p.active),
          won: parseInt(p.won), lost: parseInt(p.lost),
    ...
    res.json({
      ...
      data: {
        ...
        by_type: result.rows.reduce((acc: any, r: any) => {
          acc[r.signal_type] = {
            total: parseInt(r.total), positive: parseInt(r.positive),
...
export async function recordLearningSignal(req: Request, res: Response): Promise<void> {
  try {
    ...
    const result = await pool.query(`
      ...
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *
    `, [
      signal_type, entity_type, entity_id || null, outcome,
      JSON.stringify(feature_snapshot || {}), JSON.stringify(prediction || {}),
      agent_name || null,
      Math.abs(((prediction || {}).confidence || 0) - (['won', 'accepted', 'replied', 'completed'].includes(outcome) ? 1 : 0)),
    ]);
