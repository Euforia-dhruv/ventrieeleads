...
export default function ResultCard({ company, isSelected, onSelect, onHover }: ResultCardProps) {
        ...
        <div className="flex flex-col items-end gap-0.5 shrink-0">
          {company.lead_score !== undefined && (
            <div className="flex items-center gap-1.5">
              ...
              <div className="w-12 h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                <div
                  className={`h-full ${getScoreBarColor(company.lead_score)} rounded-full`}
                  style={{ width: `${company.lead_score}%` }}
                />
              </div>
              <span className={cn('text-[10px] font-medium w-5 text-right', getScoreColor(company.lead_score))}>
                {company.lead_score}
              </span>
            </div>
          )}
          {company.website_score !== undefined && (
            <div className="flex items-center gap-1.5">
              ...
              <div className="w-12 h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                <div
                  className={`h-full ${getScoreBarColor(company.website_score)} rounded-full`}
                  style={{ width: `${company.website_score}%` }}
                />
              </div>
              <span className={cn('text-[10px] font-medium w-5 text-right', getScoreColor(company.website_score))}>
          ...
          {company.seo_score !== undefined && (
            <div className="flex items-center gap-1.5">
              ...
              <div className="w-12 h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                <div
                  className={`h-full ${getScoreBarColor(company.seo_score)} rounded-full`}
                  style={{ width: `${company.seo_score}%` }}
                />
              </div>
              <span className={cn('text-[10px] font-medium w-5 text-right', getScoreColor(company.seo_score))}>
                {company.seo_score}
              </span>
            </div>
          )}
        ...
        <div className="mt-2 pt-2 border-t border-white/[0.04] space-y-2 animate-fade-in">
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            {company.opportunity_score !== undefined && (
              <ScoreBar label="Opportunity" score={company.opportunity_score} />
            )}
          </div>
